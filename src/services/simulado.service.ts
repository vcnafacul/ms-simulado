...
export class SimuladoService {
  ...
  public async Create(idTipo: string): Promise<number | null> {
    try {
      const oldSimulado = await this.simuladoModel.findOne({ tipo: idTipo, bloqueado: false});
      if(!!oldSimulado) {
        oldSimulado.bloqueado = true
        oldSimulado.save()
      }
      const tipo = await this.tipoSimuladoModel.findById(idTipo)
      const questoes = await this.questaoService.GetManyByRules(tipo as TipoSimulado)
  ...
  public async GetDefaults() {
    ...
    await Promise.all(tipoDefaults.map(async (tipoNome) => {
      const tipo = await this.tipoSimuladoModel.findOne({ nome: new RegExp(tipoNome, 'i') }).select("_id");
      const simulado = await this.simuladoModel.findOne({ tipo: tipo._id.toString() || "", bloqueado: false})

      if(simulado){
        const key = toPascalCaseSemAcentos(simulado?.descricao.replace("Simulado de", ""))
  ...
  public async GetById(id: string): Promise<Simulado | null> {
    return await this.simuladoModel.findById(id)
  ...
  public async Delete(id: string): Promise<boolean> {
    const update = await this.simuladoModel.updateOne({ _id: id }, { $set: { bloqueado: true }})
    return update.modifiedCount > 0
  }

  public async GetToAnswer(simuladoId: string, userId: number): Promise<SimuladoDto | null> {
    try {
  ...
  public async ReceiveAnswers(resposta: RespostaSimuladoDto) : Promise<Relatorio> {
    ...
    const resRel = simulado?.questoes.map(q => {
      const res = resposta.respostas.find(r => r.idQuestao === q._id.toString());
      return {
        questao: q._id,
  ...
  private async GetSimuladoById(id: string, inicio: Date): Promise<SimuladoDto | null> {
    const simulado =  await this.simuladoModel.findOne({ _id: id, bloqueado: false})
      .populate(['tipo', 'questoes'])
      .populate({ path: 'questoes', 
        populate: ['frente1', 'frente2','frente3', 'materia']})
  ...
  private async SimulatePerformance(respostas: RespostaRelatorio[]) : Promise<number> {
    const acertos = 
      respostas.filter((res) => res.respostaEstudante === res.alternativaCorreta).length;
